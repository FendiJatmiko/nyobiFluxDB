...
func main() {
	...
	for _, cluster := range clusters {
		log.Printf("Mean vlaues: cluster='%s', cpu_usage='%f'", cluster, meanCpuUsage(c, cluster))
	}
...
func meanCpuUsage(c client.Client, cluster string) float64 {
	q := client.Query{
		Command: fmt.Sprintf("select mean(cpu_usage) from node_status where cluster = '%s'", cluster), Database: database,
	}
